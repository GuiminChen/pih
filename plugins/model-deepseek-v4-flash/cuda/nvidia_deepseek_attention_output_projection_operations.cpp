...
Result<NvidiaDeepSeekAttentionOutputProjectionOperations>
NvidiaDeepSeekAttentionOutputProjectionOperations::Create(
  ...
  return NvidiaDeepSeekAttentionOutputProjectionOperations(
      retained_context, &async_api, kernel_api);
}
Status NvidiaDeepSeekAttentionOutputProjectionOperations::require_context() const {
...
Status NvidiaDeepSeekAttentionOutputProjectionOperations::validate_host_error(
    std::uint32_t* host) {
  if (host == nullptr) return Status::InvalidArgument("DeepSeek output projection host error is null");
  if (async_api_ != nullptr)
...
Status NvidiaDeepSeekAttentionOutputProjectionOperations::zero_u32_async(
    std::uintptr_t device, std::uintptr_t stream) {
  if (device == 0 || stream == 0)
    return Status::InvalidArgument("DeepSeek output projection clear is invalid");
  if (async_api_ != nullptr)
...
Status NvidiaDeepSeekAttentionOutputProjectionOperations::quant(
...
Status NvidiaDeepSeekAttentionOutputProjectionOperations::gemm(
...
Status NvidiaDeepSeekAttentionOutputProjectionOperations::rotary(
...
Status NvidiaDeepSeekAttentionOutputProjectionOperations::grouped_gemm(
    DeepSeekGroupedFp8GemmLaunch launch) {
  auto status = require_context(); if (!status.ok()) return status;
  if (kernels_ == nullptr) {
    return Status::FailedPrecondition("Kernel Pack capability is unavailable");
  }
  const pih_deepseek_grouped_fp8_gemm_launch_v1 request{
      sizeof(pih_deepseek_grouped_fp8_gemm_launch_v1),
      PIH_DEEPSEEK_KERNELS_ABI_VERSION_V1, launch.input_bf16,
      launch.activation_e4m3, launch.activation_scale_bits,
      launch.weight_e4m3, launch.weight_scale_bits, launch.output_bf16,
      launch.error_flag_u32, launch.stream, launch.token_count,
      launch.group_count, launch.output_per_group, launch.input_per_group};
  return deepseek_kernel_status(kernels_->launch_grouped_fp8_gemm(&request));
}
Status NvidiaDeepSeekAttentionOutputProjectionOperations::copy_error_d2h_async(
    std::uint32_t* host, std::uintptr_t device, std::uintptr_t stream) {
  if (host == nullptr || device == 0 || stream == 0)
    return Status::InvalidArgument("DeepSeek output projection error copy is invalid");
  if (async_api_ != nullptr)
