...
DeepSeekLearnedRouterPlanInputAssembler::Assemble(
  ...
  for (std::size_t index = 0; index < weights.size(); ++index) {
    ...
    if (binding.layer != input.layer) {
      return Status::FailedPrecondition(
          "DeepSeek learned router plan layer ownership is inconsistent");
    }
  ...
  if (staging->size() != weights.size()) {
    return Status::Internal(
        "DeepSeek learned router staging and weights disagree");
  }
  std::vector<DeepSeekLearnedRouterLayerPlanWork> result;
  result.reserve(weights.size());
  for (std::size_t index = 0; index < weights.size(); ++index) {
    const auto& binding = weights.at(index);
    const auto& input = layer_inputs[index];
    auto& layer_staging = staging->at(index);
    if (binding.layer != layer_staging.layer || binding.layer != input.layer ||
        input.input_bf16 == 0 ||
    ...
    if (host_error == nullptr) {
      return Status::FailedPrecondition(
          "DeepSeek learned router host error flag is unavailable");
    }
    DeepSeekLearnedRouterSubmission submission{
        binding.layer, token_count, input.input_bf16, binding.weight_bf16,
        device.scores_f32, device.error_flag_u32, stream, completion_event,
        *host_scores, host_error};
