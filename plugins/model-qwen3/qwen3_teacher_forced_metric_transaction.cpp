...
Result<QwenTeacherForcedMetricTransaction>
QwenTeacherForcedMetricTransaction::Create(
  ...
  if (stream == 0 || event_generation == 0 || owning_rank < 0 ||
      ...
      metric_plan.submitted() || metric_plan.rows() == 0 ||
      ...
      device_error.dim(0) != 1 || device_error.device().type() != DeviceType::kCuda ||
      device_error.device().index() != owning_rank ||
      device_error.generation() == 0) {
    return Status::InvalidArgument("Qwen metric transaction identity is invalid");
  }
  auto status = result_layout.initialize(pinned_result);
  if (!status.ok()) return status;
  return QwenTeacherForcedMetricTransaction(
      std::move(transfer), std::move(metric_plan), device_error, result_layout,
      std::move(completion_slot), std::move(completion_frontier), pinned_result,
      stream, event_generation, owning_rank, health_provider);
}

Status QwenTeacherForcedMetricTransaction::submit(
    TypedCopyDriver& copy_driver,
    QwenBf16DeviceErrorClearDriver& clear_driver,
    KernelLaunchDriver& kernel_driver, CompletionEventDriver& event_driver) {
  if (state_ != QwenTeacherForcedMetricTransactionState::kPrepared)
    return Status::FailedPrecondition("Qwen metric transaction cannot submit");
  auto status = transfer_.submit_upload(copy_driver);
  if (status.ok())
  ...
  if (!status.ok()) {
    state_ = QwenTeacherForcedMetricTransactionState::kPoisoned;
    return status;
  }
  state_ = QwenTeacherForcedMetricTransactionState::kSubmitted;
  return Status::Ok();
}

Result<CompletionPublicationEvidence>
QwenTeacherForcedMetricTransaction::collect() {
...
QwenTeacherForcedMetricTransaction::poll(CompletionEventDriver& event_driver) {
  if (state_ != QwenTeacherForcedMetricTransactionState::kSubmitted)
    return Status::FailedPrecondition("Qwen metric transaction is not pending");
  ...
  state_ = QwenTeacherForcedMetricTransactionState::kCompleted;
  return result;
}

Status QwenTeacherForcedMetricTransaction::expire(std::uint64_t now_ns) {
  if (state_ != QwenTeacherForcedMetricTransactionState::kSubmitted)
    return Status::FailedPrecondition("Qwen metric transaction is not pending");
...
Status QwenTeacherForcedMetricTransaction::release_completion() {
  if (state_ != QwenTeacherForcedMetricTransactionState::kCompleted)
    return Status::FailedPrecondition("Qwen metric completion is not releasable");
