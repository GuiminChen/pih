...
bool endpoint_matches(const CudaCopyEndpoint& endpoint,std::uintptr_t base,
    std::uint64_t bytes,CudaCopyMemoryType type,std::int32_t rank) {
...
Result<QwenInt4PackedSynchronousBackend>
QwenInt4PackedSynchronousBackend::Create(
  ...
  auto layout=QwenBf16PackedResultLayout::Create(identity.sample_capacity);
  if(!layout.ok() || !drivers.copy || !drivers.clear || !drivers.kernels ||
     !drivers.lm_head || !drivers.events || !drivers.health || !drivers.clock ||
     !drivers.waiter || identity.epoch==0 ||
     ...
     int4_functions.size()!=QwenInt4KernelBundle::kFunctionCount ||
     ...
     arenas.pinned_result_backing.size()!=layout->total_bytes())
    return Status::InvalidArgument("packed INT4 backend configuration is invalid");
  ...
  const auto sampled_bytes=layout->device_error().offset_bytes;
  if(!endpoint_matches(arenas.pinned_staging,staging_base,
       arenas.pinned_staging_backing.size(),
       CudaCopyMemoryType::kRegisteredPinnedHost,identity.owning_rank) ||
     ...
     !endpoint_matches(arenas.device_staging,arenas.device.step_staging.base,
       arenas.device.step_staging.bytes,CudaCopyMemoryType::kDevice,
       identity.owning_rank) ||
     !endpoint_matches(arenas.sampled_tokens,arenas.device.sampled_token.base,
       sampled_bytes,CudaCopyMemoryType::kDevice,identity.owning_rank) ||
     !endpoint_matches(arenas.device_error,arenas.device.device_error.base,
       sizeof(std::uint32_t),CudaCopyMemoryType::kDevice,identity.owning_rank))
    return Status::InvalidArgument("packed INT4 copy arenas do not match");
  return QwenInt4PackedSynchronousBackend(commands,int4_functions,
      packed_functions,weights,arenas,identity,drivers);
}

Result<QwenBf16PackedBatchExecutionView>
QwenInt4PackedSynchronousBackend::execute_packed(
  ...
  if(state_!=QwenInt4PackedBackendState::kReady)
    return Status::FailedPrecondition("packed INT4 backend is not ready");
  if(bindings.size()!=plan.sequence_count() ||
     append_plans.size()!=plan.sequence_count() ||
     sampling.size()!=plan.sequence_count())
    return Status::InvalidArgument("packed INT4 sequence inputs drifted");
  if(request_generation_==UINT64_MAX || event_generation_==UINT64_MAX ||
     plan_id_>UINT64_MAX-14){state_=QwenInt4PackedBackendState::kPoisoned;
    return Status::ResourceExhausted("packed INT4 identity exhausted");}
  state_=QwenInt4PackedBackendState::kRunning;
  const auto fail=[this](Status status)
      ->Result<QwenBf16PackedBatchExecutionView>{
    state_=QwenInt4PackedBackendState::kPoisoned;return status;};
  const auto request_generation=request_generation_++;
  const auto event_generation=event_generation_++;
  const auto upload_plan=plan_id_,readback_plan=plan_id_+12;plan_id_+=14;
  auto owners=arenas_.device;owners.device_error.generation=request_generation;
  auto built=QwenInt4PackedStepBuilder::Create(plan,metadata,kv_metadata,
      request_generation,identity_.owning_rank,identity_.slot_count,
      identity_.rms_epsilon,identity_.attention_scale,*commands_,int4_,packed_,
      *weights_,owners,arenas_.pinned_staging_backing,sampling);
  if(!built.ok())return fail(built.status());
  auto device_staging=arenas_.device_staging;
  device_staging.generation=owners.step_staging.generation;
  auto upload=QwenBf16PackedStepUpload::Create(built->staging_layout(),
      arenas_.pinned_staging,device_staging,identity_.context_identity,
      identity_.stream,event_generation,upload_plan);
  ...
  auto readback=QwenBf16PackedReadback::CreateSampling(*layout,samples,
      sampled,error,arenas_.pinned_result,identity_.context_identity,
      identity_.stream,event_generation,readback_plan);
  ...
  for(const auto& append:append_plans)committed=(std::max)(committed,
      static_cast<std::uint64_t>(append.target_committed_tokens));
  auto frontier=CudaCompletionFrontier::Create(
      {identity_.epoch,static_cast<std::uint32_t>(identity_.owning_rank),
       request_generation,plan.phase()==PackedTokenPhase::kPrefill
         ?CudaCompletionPhase::kPrefill:CudaCompletionPhase::kDecode,committed},
      event_generation,*now,*deadline);
  if(!frontier.ok())return fail(frontier.status());
  auto transaction=QwenInt4PackedStepTransaction::Create(std::move(*upload),
      built->compute(),std::move(*readback),*layout,samples,std::move(*slot),
      std::move(*frontier),arenas_.pinned_result_backing,identity_.stream,
      event_generation,*drivers_.health);
  if(!transaction.ok())return fail(transaction.status());
  auto submitted=transaction->submit(*drivers_.copy,*drivers_.clear,
      *drivers_.kernels,*drivers_.lm_head,*drivers_.events);
  if(!submitted.ok())return fail(submitted);
  for(;;){
    auto time=drivers_.clock->now_ns();if(!time.ok())return fail(time.status());
    ...
    if(receipts.ok()){
      ...
      sampling_receipts_=std::move(*receipts);sampled_tokens_.clear();
      ...
      state_=QwenInt4PackedBackendState::kReady;
      return QwenBf16PackedBatchExecutionView{sampled_tokens_,
          {identity_.event,event_generation},sampling_receipts_};
    }
    if(receipts.status().code()!=StatusCode::kUnavailable)
      return fail(receipts.status());
    auto waited=drivers_.waiter->wait();if(!waited.ok())return fail(waited);
  }
}
