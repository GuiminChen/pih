...
Result<DeepSeekExpertDeviceSpan> resolve_segment(
    ...
    const DeepSeekDsparkResidentExpertBindings::Resolver& resolver,
  ...
  if (view->data() == nullptr || view->dtype() != dtype || view->rank() != 2 ||
    ...
    return Status::FailedPrecondition(
        "DeepSeek DSpark resident expert violates checkpoint ABI");
  }
...
Result<DeepSeekExpertMatrixDeviceView> resolve_matrix(
    DeepSeekDsparkStageId stage, std::uint32_t expert,
  ...
  const auto prefix = *namespace_name + ".ffn.experts." +
...
Result<DeepSeekDsparkResidentExpertBindings>
DeepSeekDsparkResidentExpertBindings::Resolve(
  ...
  if (!stage.owns_dspark || !stage.owns_lm_head ||
    ...
    return Status::InvalidArgument(
        "DeepSeek DSpark resident expert stage identity is invalid");
  }
  DeepSeekDsparkResidentExpertBindings result;
  for (std::uint32_t index = 0; index < kDeepSeekDsparkStageCount; ++index) {
    ...
    for (std::uint32_t expert = 0; expert < kExpertCount; ++expert) {
      auto w1 = resolve_matrix(*stage_id, expert, "w1", resolver,
                               result.generation_);
      if (!w1.ok()) return w1.status();
      auto w2 = resolve_matrix(*stage_id, expert, "w2", resolver,
                               result.generation_);
      if (!w2.ok()) return w2.status();
      auto w3 = resolve_matrix(*stage_id, expert, "w3", resolver,
                               result.generation_);
      if (!w3.ok()) return w3.status();
      result.experts_[index][expert] = {*w1, *w2, *w3};
    }
  }
  return result;
}
...
DeepSeekDsparkResidentExpertBindings::Resolve(
  ...
  if (result->generation() != arena.generation()) {
    return Status::FailedPrecondition(
        "DeepSeek DSpark resident expert generation is foreign");
  }
  return result;
}
