...
class NvidiaDeepSeekExpertCudaOperations final
    : public DeepSeekExpertCudaOperations {
 public:
  static Result<NvidiaDeepSeekExpertCudaOperations> Create(
  ...
  Status validate_host_staging(
      const DeepSeekExpertHostStaging& staging) override;
  ...
  Status copy_d2h_async(void* host, std::uintptr_t device,
                        std::size_t bytes, std::uintptr_t stream) override;
  Status gather(DeepSeekRouteGatherLaunch launch) override;
  Status quantize(DeepSeekFp8ActivationQuantLaunch launch) override;
  Status gemm(DeepSeekFp4GemmLaunch launch) override;
  Status swiglu(DeepSeekExpertSwiGluLaunch launch) override;
  Status accumulate(DeepSeekExpertAccumulateLaunch launch) override;
 ...
 private:
  NvidiaDeepSeekExpertCudaOperations(
